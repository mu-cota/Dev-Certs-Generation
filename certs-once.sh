#!/bin/bash
# Script to set up directory structure and generate TLS certificates 
...
openssl req -x509 -new -nodes -key ca.key -sha256 -days 732 -out ca.crt \
  -subj "/C=SG/ST=State/L=City/O=Organization/CN=Dev CA"

# Generate vault-0 key and CSR
openssl genrsa -out vault-0.key 4096
openssl req -new -key vault-0.key -out vault-0.csr \
  -subj "/C=SG/ST=State/L=City/O=Organization/CN=vault-0"
...
openssl x509 -req -in vault-0.csr -CA ca.crt -CAkey ca.key -CAcreateserial \
  -out vault-0.crt -days 372  -sha256 -extfile vault-0.ext
